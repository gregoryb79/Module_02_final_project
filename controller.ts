import { Expense, Income, addUser, getPassword, setCurrentUser, importFromCSV, addExpense, addIncome} from "./model.js";
...
export function onIncomeSubmit (formData: FormData){
    const dateRaw = formData.get("date").toString();    
    if (!dateRaw) {
        throw new Error("Date can't be empty");
    }       
