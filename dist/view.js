...
import { getCurrentUser, getExpensesByDates, getIncomesByDates, getExpensesByCategories } from "./model.js";
...
export function index(monthInput, balanceSheet, expenseMeterCanvas, balance, percentage, expenseGraphCanvas, largestCategories, recentExpensesList) {
    ...
    function drawMeter(income, expense) {
    ...
    function renderBalanceSection(income, expense) {
...
export function expenses(filesForm, datesForm, expensesList) {
    const today = new Date();
    const monthAgo = new Date();
    monthAgo.setMonth(monthAgo.getMonth() - 1);
